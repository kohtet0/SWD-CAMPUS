...
const arrSliceLength = (array, length) => {
  const arr = [];
...
// console.log(arrSliceLength(points, 2));

const arrSlice = (array, start = 0, length = 5) => {
  const arr = [];
  for (let i = 0; i < length; i++) {
    arr[i] = array[start];
  }
...
// console.log(arrSlice(points, 3, 1));
...
console.log(arrCopy(points));

// declarative

const deArrTotal = points.reduce((pv, cv) => pv + cv, 0);

// console.log(deArrTotal);

const deEven = (array) => array.filter((el) => el % 3);

// console.log(deEven(points));

const deOdd = (array) => array.filter((el) => el % 2);

// console.log(deOdd(points));
